...
export function useCurrencySingle(symbol?: string) {
  ...
  >({
    queryKey: ['get-currency-single', selectedSymbol],
    queryFn: getCurrencySingle(selectedSymbol as string),
    enabled: !!(router.isReady && selectedSymbol),
  });

  if (query.isSuccess) {
    if (selectedSymbol === 'irt' || selectedSymbol === 'IRT')
      router.push('/404');
  }
