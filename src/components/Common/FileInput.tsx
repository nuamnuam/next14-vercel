...
const FileInput: FC<Props> = ({
  ...
  const fileName = useMemo(() => {
    if (value?.name) {
      ...
      if ((value.name.length || 0) > 30) {
        const name = value.name.slice(0, 30).trim().replaceAll('.', '') || '';
        return `${name}.${format}`;
      }
      return value?.name;
    }
  ...
  return (
    <div className="border border-dark-100 rounded-md mb-6 md:mb-0">
      <div
        className={clsx(
          'flex flex-col md:flex-row w-full md:items-center justify-between px-4 pb-4 md:py-2',
          className,
        )}
      >
        <div className="flex gap-x-2 items-center md:justify-center py-4">
          {icon && <Icon icon={icon} className={iconClassName} size={16} />}
          <p className={clsx('text-sm text-dark-600 ')}>{label}</p>
        </div>
        <input
        ...
        <div className="border-t-2 border-t-dark-50 py-4 px-4 flex flex-col md:flex-row gap-2 md:gap-0 items-start md:items-center justify-between">
          <div className="flex justify-start items-center gap-x-2">
            <Icon
              ...
              size={15}
            />
            <p className="text-xs text-primary-600">{fileName}</p>
          </div>
