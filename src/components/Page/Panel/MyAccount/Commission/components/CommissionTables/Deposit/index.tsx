...
const Deposit = () => {
    ...
    return data.result?.map((item) => ({
        ...
        <div className="flex flex-col items-center gap-2 text-sm text-dark-600 justify-between h-full">
          {Object.values(item.networks ?? {})?.map((network) => (
            <div className="flex gap-1">
              <span>{toPrice(network.deposit_fee)}</span>
      ...
      minDeposit: (
        <div className="flex flex-col items-center gap-2 text-sm text-dark-600 justify-between h-full">
          {Object.values(item.networks ?? {})?.map((network) => (
            <div className="flex gap-1">
              <span>{toPrice(network.min_deposit)}</span>
              <span className="lg:block hidden">{item.symbol}</span>
            </div>
          ))}
        </div>
      ...
      status: (
        <div className="flex flex-col items-end gap-2 lg:pl-4 text-sm text-dark-600 lg:items-center lg:p-0">
          {Object.values(item.networks ?? {})?.map((network) => (
            <Chip
              label={
                network.is_depositable ? commisson.active : commisson.deactive
              }
              size="sm"
              variant={network.is_depositable ? 'success' : 'danger'}
            />
          ))}
        </div>
      ),
  ...
  return (
    <div>
      ...
      <div className="p-4 md:p-6 flex items-center justify-between border-b border-dark-50 md:border-none">
        <h2 className="text-base text-dark-800 font-normal hidden lg:block">
          {commisson.depositFeeList}
        </h2>
        <FormInput
          size="sm"
          ...
          <div className="p-4 sm:p-6 border-b border-dark-50" key={index}>
            ...
            {Object.values(item.networks ?? {})?.map((net) => (
              <div className="border border-dark-50 rounded-lg p-4 mt-4">
                ...
                <LabelValue
                  label={`${commisson.depositFee}:`}
                  value={`${item.symbol} ${toPrice(net.deposit_fee)}`}
                  classNames="mb-2"
                />
                <LabelValue
                  label={`${commisson.minDeposit}:`}
                  value={`${item.symbol} ${toPrice(net.min_deposit)}`}
                />
              </div>
            ))}
          </div>
