...
import {
  MigrateUserPayload,
  useMigrateUser,
} from '@/requests/panel/my-account/kyc/migrateUser';
...
const PersonalInfoForm = () => {
  ...
  const { mutateAsync: mutateMigrateUser, isPending: isLoading } =
    useMigrateUser();
    ...
    try {
      ...
      const body: MigrateUserPayload = {
        ...
        birthday:
          ...
          '/' +
          (+day < 10 ? '0' + day : day),
        national_code: toEnglishDigits(national_code)?.trim(),
      };

      await mutateMigrateUser(body).then(() => {
